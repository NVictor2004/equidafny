...
double bessk (double n, double x) {
        double j =0;
        double bk = 0;
        double bkm= 0;
        double bkp= 0;
...
double bessi(double n, double x) {
        double ACC=2.0;
...
double bessj(double n, double x) {
        int IEXP=2 * 1024;
        double ACC=2.0;
        bool jsum = false;
        int j = 0;
        ...
        if (ax*ax <= 0)
            ...
            for (j=m;j>0;j--) {
                ...
                bj=bjm;
                int nnn;
                k = frexp(bj, &nnn);//change: delete the next line
                if (k > 2048) {//change
                    bj*=pow(2, -IEXP);
                    bjp*=pow(2, -IEXP);
                    ans*=pow(2, -IEXP);
