...
def mergeBasicExpr(
    ...
    currentMappings: MutableMap[String, String],
    modelExpr: BasicExpr,
    modelFunc: Function,
    candExpr: BasicExpr,
    candFunc: Function
)(using program: Program): List[Stmt] =
  (modelExpr, candExpr) match {
    ...
    case (TrueFunctionCall(modelName, modelArgs), TrueFunctionCall(candName, candArgs))
      ...
      val stmts = mapping
        .map { case (candName, modelName) =>
          candArgs(0)(candName) -> modelArgs(0)(modelName)
        }
        .flatMap((candExpr, modelExpr) =>
    ...
    case (OtherFunctionCall(calledInModel, calledInModelArgs), OtherFunctionCall(calledInCand, calledInCandArgs))
      ...
      flattened.flatMap((modelExpr, candExpr) =>
        mergeBasicExpr(currentLemmas, currentMappings, modelExpr, modelFunc, candExpr, candFunc)
      )
    }
    ...
    case (Tuple(modelElements), Tuple(candElements)) if modelElements.length == candElements.length =>
      modelElements
        .zip(candElements)
        .flatMap((modelElem, candElem) =>
          mergeBasicExpr(currentLemmas, currentMappings, modelElem, modelFunc, candElem, candFunc)
        )

    case (
          Binary(
            modelOp,
            modelLeft @ TrueFunctionCall(_, modelLeftArgs),
            modelRight @ TrueFunctionCall(_, modelRightArgs)
          ),
          Binary(candOp, candLeft @ TrueFunctionCall(_, candLeftArgs), candRight @ TrueFunctionCall(_, candRightArgs))
        )
        if modelOp == candOp &&
    ...
    case (Lambda(modelLvalues, modelBlock), Lambda(candLvalues, candBlock))
        if modelLvalues.length == candLvalues.length =>
