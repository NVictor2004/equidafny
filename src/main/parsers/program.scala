...
lazy val function = Function(
    "function" ~> ident, option("<" ~> sepBy(ident <~> option("(" ~> gOption <~ ")"), ",") <~ ">"), "(" ~> parameters <~ ")", ":" ~> typeParser, spec, "{" ~> expr <~ "}"
    )

lazy val lemma = Lemma("lemma" ~> ident, option("<" ~> sepBy(ident <~> option("(" ~> gOption <~ ")"), ",") <~ ">"), "(" ~> parameters <~ ")", spec, option(block))

lazy val datatype = Datatype("datatype" ~> ident, option("<" ~> sepBy(ident <~> option("(" ~> gOption <~ ")"), ",") <~ ">"), "=" ~> sepBy(declaredType, "|"))
