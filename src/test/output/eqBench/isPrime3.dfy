...
requires (i <= |PRIMESM|)
decreases ((|PRIMESM| - i))
{if (i == |PRIMESM|) then LibM(x, 1) else if (x == PRIMESM[i]) then 1 else ClientLoopM(x, (i + 1))}
...
if ((x % p) == 0) then if (x == p) then 1 else 0 else LibLoopM(x, (i + 1))}
...
requires (i <= |PRIMES1|)
decreases ((|PRIMES1| - i))
{if (i == |PRIMES1|) then Lib1(x, 1) else if (x == PRIMES1[i]) then 1 else ClientLoop1(x, (i + 1))}
...
if ((x % p) == 0) then 0 else LibLoop1(x, (i + 1))}

lemma ClientM_Client1_Equivalence(x: nat)
ensures (ClientM(x) == Client1(x))
{{ClientLoopM_ClientLoop1_Equivalence(x, 0);}}
...
lemma ClientLoopM_ClientLoop1_Equivalence(x: nat, i: int)
requires (0 <= i)
requires (i <= |PRIMESM|)
decreases ((|PRIMESM| - i))
ensures (ClientLoopM(x, i) == ClientLoop1(x, i))
{{match (i == |PRIMESM|) {
case false =>match (x == PRIMESM[i]) {
case false =>ClientLoopM_ClientLoop1_Equivalence(x, (i + 1));
case true =>
}
