...
datatype Formula = True_ | False_ | Not(param0: Formula) | AndAlso(param0: Formula, param1: Formula) | OrElse(param0: Formula, param1: Formula) | Imply(param0: Formula, param1: Formula) | Equal(left: Exp, right: Exp)
...
case False_ => false
case Not(f) => !evalM(f)
case AndAlso(p1, p2) => var p11 := evalM(p1);
var p22 := evalM(p2);
(p11 && p22)
case OrElse(p1, p2) => var p11 := evalM(p1);
var p22 := evalM(p2);
(p11 || p22)
case Imply(p1, p2) => var p11 := evalM(p1);
var p22 := evalM(p2);
(!p11 || p22)
case Equal(p1, p2) => (exp_evalM(p1) == exp_evalM(p2))
}
}
...
case Imply(a, b) => assert (size(Not(AndAlso(a, Not(b)))) < size(Imply(a, b)));
...
lemma evalM_evalC_Equivalence(f: Formula)
decreases (size(f))
