...
decreases (pos)
{if pos == 0 then true else validListM(adjList[pos - 1], N) && validAdjListM(adjList, N, pos - 1)}function validAdjList(adjList :seq<List<int>>, N :int, pos :int): bool
requires (N >= 1 && pos >= 0 && pos <= N && N == |adjList|)
decreases (pos)
{if (pos == 0) then true else if (validAdjList(adjList, N, pos - 1)) then validList(N, adjList[pos - 1]) else false}function validList(N :int, l :List<int>): bool
requires (N >= 1)
{match l {
