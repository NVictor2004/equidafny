...
{loopM(start, next, leq, max, Nil)}function unfoldingSorted5<State, Elem>(start :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, max :int): List<Elem>
{go5(start, next, leq, Nil, max)}function unfoldingSorted4<State, Elem>(start :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, max :int): List<Elem>
{go4(start, next, leq, Nil, max)}function unfoldingSorted3<State, Elem>(start :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, max :int): List<Elem>
{go3(start, next, Nil, max)}function unfoldingSorted1<State, Elem>(start :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, max :int): List<Elem>
{go1(start, next, leq, Nil, max)}function go5<State, Elem>(s :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, xs :List<Elem>, fuel :int): List<Elem>
...
}lemma unfoldingSorted5Equivalence<State, Elem>(start :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, max :int)
ensures unfoldingSortedM(start, next, leq, max) == unfoldingSorted5(start, next, leq, max)
{{}}lemma unfoldingSorted4Equivalence<State, Elem>(start :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, max :int)
ensures unfoldingSortedM(start, next, leq, max) == unfoldingSorted4(start, next, leq, max)
{{}}lemma unfoldingSorted3Equivalence<State, Elem>(start :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, max :int)
ensures unfoldingSortedM(start, next, leq, max) == unfoldingSorted3(start, next, leq, max)
{{}}lemma unfoldingSorted1Equivalence<State, Elem>(start :State, next :State -> Option<(State, Elem)>, leq :(Elem, Elem) -> bool, max :int)
