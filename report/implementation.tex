...
Our tool is divided into several components. The most important component is the generation of Dafny data structures to facilitate the equivalence proof; this is described in the following sections. Similarly, the evaluation component is described in \Cref{chp:evaluation}. The remaining components are described in this section. 
...
\subsection{Merging each type of function body}
The bodies of Dafny functions can be composed of a variety of different types of structures. Function Merging for some structures is complex, so I have described them in detail in this section. 

\paragraph{Function Calls} During the merging of a pair of functions, a pair of calls to helper functions may be encountered. If this occurs, the helper functions themselves must be merged in a recursive manner. Once this completes, this results in an equivalence lemma and a set of mappings between the parameters of the helper functions. These mappings can be used to create mappings between the corresponding arguments in the calls to the helper functions. These arguments must themselves be merged together to identify further calls to helper functions and to create further mappings between the parameters of the original functions. \footnote{Example needed?}

\paragraph{Match Expressions} TODO \footnote{Write this when match expressions are finished}

\section{Technical Audit Limitations}

In \Cref{sec:audit}, I demonstrated that Dafny has several types of limitations when attempting to prove program equivalence. In this section, I will describe how our tool attempts to remove most of the limitations identified. The process of generating and calling equivalence lemmas for helper functions has already been described in \Cref{sec:worked-example} (the limitation this refers to is described in \Cref{sec:equivalence-lemmas}). 
...
\footnote{Finish this induction section}
...
\footnote{Also need to copy termination lemmas and assertions}
