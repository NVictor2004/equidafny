...
Parameter Mappings are generated in multiple stages. First, if only a single parameter in each function is of a specific type, then these parameters must be mapped together. For example, given the following signatures for the functions \lstinline|f| and \lstinline|g|:
\begin{minted}{lexer.py:CustomLexer}
function f(a: int, b: bool, c: bool): int
function g(x: bool, y: int, z: bool): int
\end{minted}
%
Parameters \lstinline|a| and \lstinline|y| can immediately be mapped together, since they are each the only \lstinline|int| parameter in their respective functions. However, parameters \lstinline|b|, \lstinline|c|, \lstinline|x| and \lstinline|z| cannot be mapped in this way. Parameter mappings are stored by mapping candidate function parameters to model function parameters.
...
\end{minted}
%
These functions rely on helper functions \lstinline|insertM| and \lstinline|insert1| that each insert a single integer into a sorted list while maintaining its order. Their implementations are in \Cref{sec:intro-background}. Thus, the arguments in the function calls to \lstinline|insertSortedM| and \lstinline|insertSorted1| must themselves be merged to discover that an equivalence lemma between the helper functions \lstinline|insertM| and \lstinline|insert1| is necessary. This results in the following equivalence lemmas:
