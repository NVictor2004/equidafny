...
Therefore, since the proof of the equivalence of \lstinline|unique| and \lstinline|distinct| relies on the equivalence of \lstinline|find| and \lstinline|isin|, the user must explicitly call the equivalence lemma for the helper functions, with the correct arguments, within the main equivalence lemma (as demonstrated above). 
...
\end{lstlisting}
%
As demonstrated above, any necessary helper equivalence lemmas must be explicitly called by the user, with the correct arguments.

\footnote{Add more stuff here for argument matching for the chk function}
...
\end{lstlisting}
%
Sometimes, these helper lemmas can only be called when specific conditions are met. To demonstrate this second point, I will provide two implementations of a function to determine whether a given adjacency list is valid. They are as follows:
