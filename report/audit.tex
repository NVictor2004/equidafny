...
However, it incorrectly provides \lstinline|Cons(-38, Cons(7681, Nil))| as a counterexample. As found by \textcite{putrueleMaskDToolMeasuring2022}, counterexamples may sometimes be incorrect due to the Z3 solver being incomplete. I extended the above proof to include and use a helper lemma, as shown below:
\begin{minted}{lexer.py:CustomLexer}
...
% Caption: lstinline|isin| and \lstinline|find| Functions
\begin{minted}{lexer.py:CustomLexer}
...
\end{minted}
%
This implementation can be converted into the following imperative method. It is not an exact conversion from the recursive version, to make better use of imperative features. Both the function and the method use sequences, so that I can use Dafny's built-in indexing and slicing features. The method's postcondition states that it will output a boolean equivalent to whether each element in the sequence is less than or equal to the next element. If this can be proven, then the sequence must be sorted in non-decreasing order. The variable \lstinline|index| together with the invariant in the \lstinline|while| loop are used to keep track of which sequence elements have been checked so far.
\begin{minted}{lexer.py:CustomLexer}
...
For example, we can use this process to prove the equivalence of the imperative versions of \lstinline|isSortedR| and \lstinline|isSortedB| (the function version of \lstinline|isSortedR| can be found in \Cref{sec:counterexamples}). The imperative version of \lstinline|isSortedR| (called \lstinline|isSortedRI|) is as follows, requiring a helper function \lstinline|loopI|. As above, the methods are not an exact conversion from their recursive counterparts. Additionally, the \lstinline|isSortedR| function operates on Lists, so I have converted it to use sequences instead. 

The helper method \lstinline|loopI| returns a boolean representing whether an integer \lstinline|p| prepended to a list of integers \lstinline|l| is a list sorted in non-decreasing order. The function's postcondition verifies this by finding whether the list \lstinline|l| is sorted, and whether \lstinline|p| is less than or equal to the first value in \lstinline|l| (if \lstinline|l| is non-empty). \lstinline|loopI| implements this by maintaining a `rolling' comparison of adjacent list elements. It starts by comparing \lstinline|p| and the first element of \lstinline|l|, and continues by comparing adjacent elements of \lstinline|l|. The variable \lstinline|next| represents how far the loop has progressed through the list \lstinline|l|. The \lstinline|isSortedRI| method calls the helper method \lstinline|loopI| for non-empty lists and returns \lstinline|true| for empty lists:
...
A similar problem to that discussed above is proving the equivalence of a method and a function. In this case, the user does not need to write a postcondition for the method, since the function can be called directly within the specification. The user still needs to provide loop invariants and variants for the method, however, these do not need to be as complicated as those detailed above, as again, they can be defined in terms of the function. For example, the function \lstinline|isSortedB| defined in \Cref{sec:imperative-equivalence} can be proven to be equivalent with the following imperative method:
\begin{minted}{lexer.py:CustomLexer}
method isSortedBI(l: seq<int>) returns (res: bool)
...
Dafny is able to prove the equivalence of some higher-order functions, but not all. For example, here are two functions \lstinline|andThen1| and \lstinline|andThen2|, which compute function composition:
\begin{minted}{lexer.py:CustomLexer}
...
\end{minted}
%
Here are two more such functions \lstinline|flip1| and \lstinline|flip2|, which each take a binary function as input, and output a function that computes the same output, but with the arguments flipped:
\begin{minted}{lexer.py:CustomLexer}
